...
std::vector<double> RegressionCV::doCVRegression( int seed, char* path,
                                            char* name, int mpi_rank )
{
    std::vector<double> cv_stats;

    #ifdef AVX_VERSION
        double* lastCV = (double*)aligned_alloc( ALIGNMENT, lengthGamma * sizeof(double));
    #else
    ...
    for( int j=0; j<lengthLambda; j++ )
    {
                    ...
                    if( cv_data[i][f].wCV[ii] != 0.0 )
                    {
                        for( int l=0; l<K; ++l)
                            cv_predict[i][INDEX(ii,l,memory_N)]
                                = cv_data[i][f].xTimesBeta[INDEX(ii,l,memory_N)];
                    }
                }
            }
        }
           ...
           cvErrorSD  = sqrt( cvErrorSD );
            ...
            if( path != NULL )
            {
                ...
                for( int k=0; k<K; k++ )
                {
                    ...
                    for( int jj=0; jj<P; jj++ )
                        fprintf( file, "%a,", cv_data[i][nFold].beta[INDEX(jj,k,memory_P)] );
                }
            ...
            else
            {
                ...
                for( int k=0; k<K; k++ )
                {
                    ...
                    for( int p=0; p<P; p++ )
                        cv_stats.push_back( cv_data[i][nFold].beta[INDEX(p,k,memory_P)] );
                }
        ...
        if( worseLambdaSteps > cvStop ) break;
    }
