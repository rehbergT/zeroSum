...
int RegressionDataScheme::cdMove( int k, int l )
{
    ...
    double ak = sum_a_times_b( xk, tmp_array1, N )
                    + lambda * ( 1.0 - alpha) * v[k];
...
int RegressionDataScheme::cdMoveFused( int k, int l )
{
    double* betaL = &beta[ INDEX(0,l,memory_P) ];
    double* betak = &betaL[k];
    double* xk = &x[INDEX(0,k,memory_N)];
    ...
    double bk1 = sum_a_add_b_times_c( xb, tmp_array2, tmp_array1, N );
    double c1 = lambda * alpha * v[k];
    ...
    beta_tmp[5] += 2.0 * gamma;
    beta_tmp[6] -= 2.0 * gamma;
    ...
    beta_tmp[13] += 2.0 * gamma;
    beta_tmp[14] -= 2.0 * gamma;
    ...
    if(      beta_tmp[0]<0 && k==0 && beta_tmp[0]-betaL[1] > 0.0 )
    {
        bk = beta_tmp[0];
    }
    else if( beta_tmp[1]<0 && k==0 && beta_tmp[1]-betaL[1] < 0.0 )
    {
        bk = beta_tmp[1];
    }
    else if( beta_tmp[2]<0 && k==P && betaL[P-1] - beta_tmp[2] > 0.0 )
    {
        bk = beta_tmp[2];
    }
    else if( beta_tmp[3]<0 && k==P && betaL[P-1] - beta_tmp[3] < 0.0 )
    {
        bk = beta_tmp[3];
    }
    else if( beta_tmp[4]<0 && k!=0 && k!=P && betaL[k-1]-beta_tmp[4] > 0.0 && beta_tmp[4]-betaL[k+1] > 0.0 )
    {
        bk = beta_tmp[4];
    }
    else if( beta_tmp[5]<0 && k!=0 && k!=P && betaL[k-1]-beta_tmp[5] > 0.0 && beta_tmp[5]-betaL[k+1] < 0.0 )
    {
        bk = beta_tmp[5];
    }
    else if( beta_tmp[6]<0 && k!=0 && k!=P && betaL[k-1]-beta_tmp[6] < 0.0 && beta_tmp[6]-betaL[k+1] > 0.0 )
    {
        bk = beta_tmp[6];
    }
    else if( beta_tmp[7]<0 && k!=0 && k!=P && betaL[k-1]-beta_tmp[7] < 0.0 && beta_tmp[7]-betaL[k+1] < 0.0 )
    {
        bk = beta_tmp[7];
    }
    else if( beta_tmp[8]>0 && k==0 && beta_tmp[8]-betaL[1] > 0.0 )
    {
        bk = beta_tmp[8];
    }
    else if( beta_tmp[9]>0 && k==0 && beta_tmp[9]-betaL[1] < 0.0 )
    {
        bk = beta_tmp[9];
    }
    else if( beta_tmp[10]>0 && k==P && betaL[P-1] - beta_tmp[10] > 0.0 )
    {
        bk = beta_tmp[10];
    }
    else if( beta_tmp[11]>0 && k==P && betaL[P-1] - beta_tmp[11] < 0.0 )
    {
        bk = beta_tmp[11];
    }
    else if( beta_tmp[12]>0 && k!=0 && k!=P && betaL[k-1]-beta_tmp[12] > 0.0 && beta_tmp[12]-betaL[k+1] > 0.0 )
    {
        bk = beta_tmp[12];
    }
    else if( beta_tmp[13]>0 && k!=0 && k!=P && betaL[k-1]-beta_tmp[13] > 0.0 && beta_tmp[13]-betaL[k+1] < 0.0 )
    {
        bk = beta_tmp[13];
    }
    else if( beta_tmp[14]>0 && k!=0 && k!=P && betaL[k-1]-beta_tmp[14] < 0.0 && beta_tmp[14]-betaL[k+1] > 0.0 )
    {
        bk = beta_tmp[14];
    }
    else if( beta_tmp[15]>0 && k!=0 && k!=P && betaL[k-1]-beta_tmp[15] < 0.0 && beta_tmp[15]-betaL[k+1] < 0.0 )
    {
        bk = beta_tmp[15];
    }
    else
...
int RegressionDataScheme::cdMoveZS( int k, int s, int l )
{
    ...
    a_add_scalar_b_times_c_sub_d_times_e( xb, *betak, xk, xs, ukus, tmp_array2, N);
...
int RegressionDataScheme::cdMoveZSRotated( int n, int m, int s, int l, double theta )
{
    ...
    a_times_scalar_b_add_c_times_scalar_d_add_d_times_scalar_f(  xm, sinT, xn, -cosT, xs, unum2, tmp_array1, N );

    double a = sum_square_a_times_b( tmp_array1, w, N ) + l2 * ( v[n] * cosT * cosT
                    + v[m] * sinT * sinT + v[s] * unum1 * unum1 );

    double b = -sum_a_times_b_times_c( w, xb, tmp_array1, N) - l2 * ( v[n] * *betan * cosT
                 - v[m] * *betam * sinT + v[s] * *betas * unum1);
    ...
    sub_a_times_scalar_b_sub_c_times_scalar_d_sub_d_times_scalar_f( xn, diffn, xm, diffm, xs, diffs, xb, N);
