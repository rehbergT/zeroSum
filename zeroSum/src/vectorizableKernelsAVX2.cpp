...
void interceptMoveKernelAVX2(double* y,
    ...
    for (uint32_t i = 0; i < N; i += 4) {
        _y = _mm256_loadu_pd(&y[i]);
        _xb = _mm256_loadu_pd(&xb[i]);

        _y = _mm256_sub_pd(_y, _xb);

        _w = _mm256_loadu_pd(&w[i]);
...
void cdMoveZSRotatedKernel2AVX2(double* y,
    ...
    for (uint32_t i = 0; i < N; i += 4) {
        ...
        _ww = _mm256_loadu_pd(&w[i]);
        _xb = _mm256_mul_pd(_ww, _xb);

        _tmp = _mm256_loadu_pd(&tmp[i]);
...
void cvMoveZSRotatedKernel3AVX2(double* xb,
    ...
    for (uint32_t i = 0; i < N; i += 4) {
        ...
        _x = _mm256_loadu_pd(&xn[i]);
        _xb = _mm256_fnmadd_pd(_x, _diffn, _xb);

        _x = _mm256_loadu_pd(&xm[i]);
        _xb = _mm256_fnmadd_pd(_x, _diffm, _xb);

        _x = _mm256_loadu_pd(&xs[i]);
...
void weightedSquareSumKernelAVX2(double* a,
                                 double* b,
                                 uint32_t N,
                                 double* res) {
    __m256d _a, _b;
...
void squareWeightedSumKernelAVX2(double* a,
                                 double* b,
                                 uint32_t N,
                                 double* res) {
    __m256d _a, _b;
