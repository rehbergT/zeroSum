...
\begin{itemize}
    ...
    \item \textbf{devianceStop}: This parameter stops the CV progress if the model's deviance becomes successively
		worse for lower lambda values. The number of steps that are accepted before the CV routine 
		cancels is given by lambdaSteps (Default: 100) times devianceStop (Default: 0.1). 
		Use devianceStop = 0 or FALSE to deactivate the devianceStop.
\end{itemize}
