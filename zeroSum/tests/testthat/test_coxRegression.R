...
test_that("cox regression test", {
    ...
    A <- zeroSum(x, y, alpha, lambda, family = "cox", standardize = FALSE,
                            weights = w, zeroSum = FALSE)
    eA <- extCostFunction(x, y, coef(A), alpha, lambda, family = "cox")

    A_LS <- zeroSum(x, y, alpha, lambda, family = "cox", standardize = FALSE,
                            algorithm = "LS", weights = w, zeroSum = FALSE)
    eA_LS <- extCostFunction(x, y, coef(A_LS), alpha, lambda, family = "cox")

    A_LS2 <- zeroSum(x, y, alpha, lambda, family = "cox", standardize = FALSE,
    ...
    FA1 <- zeroSum(x, y, alpha, lambda, algorithm = "LS", gamma = gamma,
                   fusion = fusion, family = "cox", zeroSum = FALSE)
    eFA1 <- extCostFunction(x, y, coef(FA1), alpha, lambda, family = "cox",
                gamma = gamma, fusion = fusion)

    FA2 <- zeroSum(x, y, alpha, lambda, algorithm = "LS", gamma = gamma,
                   fusion = fusion, useApprox = FALSE,
                family = "cox", zeroSum = FALSE)
    eFA2 <- extCostFunction(x, y, coef(FA2), alpha, lambda, family = "cox",
                gamma = gamma, fusion = fusion)

    FA3 <- zeroSum(x, y, alpha, lambda, algorithm = "LS", gamma = gamma,
                   fusion = fusion, family = "cox")
    eFA3 <- extCostFunction(x, y, coef(FA3), alpha, lambda, family = "cox",
                gamma = gamma, fusion = fusion)

    FA4 <- zeroSum(x, y, alpha, lambda, algorithm = "LS", gamma = gamma,
                   fusion = fusion, useApprox = FALSE, family = "cox")
    eFA4 <- extCostFunction(x, y, coef(FA4), alpha, lambda, family = "cox",
                            gamma = gamma, fusion = fusion)

    FA5 <- zeroSum(x, y, alpha, lambda, algorithm = "LS", standardize = TRUE,
                gamma = gamma, fusion = fusion, family = "cox", zeroSum = FALSE)
    eFA5 <- extCostFunction(x, y, coef(FA5), alpha, lambda, family = "cox",
                gamma = gamma, fusion = fusion)

    FA6 <- zeroSum(x, y, alpha, lambda, algorithm = "LS", standardize = TRUE,
                gamma = gamma, fusion = fusion, useApprox = FALSE,
                family = "cox", zeroSum = FALSE)
    eFA6 <- extCostFunction(x, y, coef(FA6), alpha, lambda, family = "cox",
                gamma = gamma, fusion = fusion)

    FA7 <- zeroSum(x, y, alpha, lambda, algorithm = "LS", standardize = TRUE,
                gamma = gamma, fusion = fusion, family = "cox")
    eFA7 <- extCostFunction(x, y, coef(FA7), alpha, lambda, family = "cox",
                gamma = gamma, fusion = fusion)

    FA8 <- zeroSum(x, y, alpha, lambda, algorithm = "LS", standardize = TRUE,
              gamma = gamma, fusion = fusion, useApprox = FALSE, family = "cox")
    eFA8 <- extCostFunction(x, y, coef(FA8), alpha, lambda, family = "cox",
              gamma = gamma, fusion = fusion)
    ...
    fused <- abs(as.numeric(fusion %*% cbind(coef(FA1), coef(FA2), coef(FA3),
                 coef(FA4), coef(FA5), coef(FA6), coef(FA7), coef(FA8))[-1, ]))
