...
test_that("linear regression equals glmnet", {
    ...
    FA1 <- zeroSum(x, y, alpha, lambda, algorithm = "LS", standardize = FALSE,
                   ...
                   zeroSum = FALSE)
    eFA1 <- extCostFunction(x, y, coef(FA1), alpha, lambda, family = "gaussian",
                            gamma = gamma, fusion = fusion)

    FA2 <- zeroSum(x, y, alpha, lambda, algorithm = "LS", standardize = FALSE,
                   gamma = gamma, fusion = fusion, family = "gaussian")
    eFA2 <- extCostFunction(x, y, coef(FA2), alpha, lambda, family = "gaussian",
                   gamma = gamma, fusion = fusion)

    gamma <- 0.03
    lambda <- 0.001

    FA3 <- zeroSum(x, y, alpha, lambda, algorithm = "LS", standardize = TRUE,
                   gamma = gamma, fusion = fusion, family = "gaussian",
                   zeroSum = FALSE)
    eFA3 <- extCostFunction(x, y, coef(FA3), alpha, lambda, family = "gaussian",
                    gamma = gamma, fusion = fusion)

    FA4 <- zeroSum(x, y, alpha, lambda, algorithm = "LS", standardize = TRUE,
                   gamma = gamma, fusion = fusion, family = "gaussian")
    eFA4 <- extCostFunction(x, y, coef(FA4), alpha, lambda, family = "gaussian",
                            gamma = gamma, fusion = fusion)

    costs <- c(eFA1$cost, eFA2$cost, eFA3$cost, eFA4$cost)

    for (i in 1:length(costs))
        expect_lte(costs[i], ref$test_linear$fusionCosts[i] + 5e-2)
