    ...
    test_that( "zerosum coordinate descent move seems to work",{
        ...
        zeroSumCd <- function( data, k , s )
        {
            ...
            bk1 <- ( bk - data$lambda * data$alpha*( data$v[k] - data$v[s] * data$u[k] / data$u[s]  ) )  / ak
            bk2 <- ( bk - data$lambda * data$alpha*( data$v[k] + data$v[s] * data$u[k] / data$u[s]  ) )  / ak
            bk3 <- ( bk + data$lambda * data$alpha*( data$v[k] + data$v[s] * data$u[k] / data$u[s]  ) )  / ak
            bk4 <- ( bk + data$lambda * data$alpha*( data$v[k] - data$v[s] * data$u[k] / data$u[s]  ) )  / ak

            bs1 <- ( data$cSum - data$u[-c(s,k)] %*% data$beta[-c(1,s+1,k+1)] - data$u[k] * bk1 ) / data$u[s]
            bs2 <- ( data$cSum - data$u[-c(s,k)] %*% data$beta[-c(1,s+1,k+1)] - data$u[k] * bk2 ) / data$u[s]
            bs3 <- ( data$cSum - data$u[-c(s,k)] %*% data$beta[-c(1,s+1,k+1)] - data$u[k] * bk3 ) / data$u[s]
            bs4 <- ( data$cSum - data$u[-c(s,k)] %*% data$beta[-c(1,s+1,k+1)] - data$u[k] * bk4 ) / data$u[s]
